...
This group already has a file-based memory at `CLAUDE.local.md` and a `conversations/` folder. Use both:

- **CLAUDE.local.md**: top-of-context preferences and stable per-group facts that should be loaded every turn (small, curated).
- **mnemon**: searchable graph memory — reasoning conclusions, decisions, insights, observations from many sessions. Fetched on-demand via `recall`, not auto-loaded.

When in doubt: short and always-relevant → CLAUDE.local.md; everything else → mnemon.
