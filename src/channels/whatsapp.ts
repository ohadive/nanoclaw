      ...
      sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const msg of messages) {
          try {
            ...
            // Voice-note transcription: if the message is a PTT with no text,
            // transcribe via Whisper and use the transcript as the content.
            if (!content && normalized.audioMessage?.ptt) {
              try {
                const transcript = await transcribeAudioMessage(msg);
                if (transcript) {
                  content = `[Voice: ${transcript}]`;
                  log.info('Transcribed voice message', { chatJid, length: transcript.length });
                } else {
