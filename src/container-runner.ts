...
async function spawnContainer(session: Session): Promise<void> {
  ...
  // tool can read a local snapshot. Manager-only — spokes skip the cost.
  {
    ...
    if (isManagerName(agentGroup.name)) {
      ...
      await writeAgentRoster(agentGroup.id, session.id);
    }
  }
...
export async function buildMounts(
  ...
  mounts.push({
    hostPath: fameclawDir,
    containerPath: '/home/node/.config/fameclaw',
    readonly: false,
    mountClass: 'allowlisted-extra',
    scope,
  });
...
export function composeSessionSpec(input: ComposeSessionSpecInput): SessionSpec {
  ...
  const env: Record<string, string> = {
    ...
    ...mailboxEnvironment,
    // Per-group env from container_configs (fork feature; e.g. skill config).
    // Composed lane: admission refuses secret-shaped keys/values, so this
    // carries configuration only — credentials ride the OneCLI vault.
    ...(containerConfig.env ?? {}),
  };
