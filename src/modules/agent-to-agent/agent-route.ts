...
async function resolveTargetSession(
  ...
  const targetGroup = await getAgentGroup(targetAgentGroupId);
  if (targetGroup?.console_messaging_group_id) {
    const consoleMgId = targetGroup.console_messaging_group_id;
    const wiring = await getMessagingGroupAgentByPair(consoleMgId, targetAgentGroupId);
    const mode = (wiring?.session_mode as 'shared' | 'per-thread' | 'agent-shared') ?? 'shared';
    return (await resolveSession(targetAgentGroupId, consoleMgId, null, mode)).session;
  }
