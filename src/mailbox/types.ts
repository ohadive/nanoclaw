...
export interface InboundMailbox {
  ...
  /** Most recent inbound thread id for a platform id, or null. Fork extension:
   *  lets shared-mode sessions route MCP sends to the thread the user last
   *  wrote in (see writeSessionRouting in session-manager.ts). */
  latestInboundThreadId(platformId: string): string | null;
